...
public class XcScoreManager(ILogger<XcScoreManager> logger, IConfiguration config, XcScoreManagerData xcScoreManagerData) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested && _running)
        {
            ...
            if (request is not null)
            {
                ...
                await ProcessRequest(request, flightPath, scorePath, stoppingToken);
    ...
    private async Task ProcessRequest(XcScoreManagerData.ProcessRequest request, string flightPath, string scorePath, CancellationToken stoppingToken)
    {
        try
        {
            ...
            if (xcScore != null)
            {
                ...
                await request.Db.UpdateFlightAsync(request.Flight.ToFlight());
            }
        }
